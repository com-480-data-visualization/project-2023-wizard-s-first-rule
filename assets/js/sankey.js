...
(function () { // small trick to have 'global' variable only global for this script
  ...
  let data; let
    urls;
    // diagram
  let graph; let sankey; let
    svg;
    // elements
  let nodes; let links; let labels; let
    hover;
  ...
  function get_sankeyData(raw_data) {
    ...
    regionsL.forEach((r) => data_sankey.nodes.push({
      [name_str]: r,
      [category_str]: 'regionL',
    }));
    genres.forEach((g) => data_sankey.nodes.push({
      [name_str]: g,
      [category_str]: 'genre',
    }));
    platforms.forEach((p) => data_sankey.nodes.push({
      [name_str]: p,
      [category_str]: 'platform',
    }));
    regionsR.forEach((r) => data_sankey.nodes.push({
      [name_str]: r,
